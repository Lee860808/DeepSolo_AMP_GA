...
class Trainer(DefaultTrainer):
    ...
    def resume_or_load(self, resume=True):
        ...
        print("--- DEBUG: Entering Trainer.resume_or_load ---")
        checkpoint = self.checkpointer.resume_or_load(self.cfg.MODEL.WEIGHTS, resume=resume)
        print(self.model.detection_transformer.ctrl_point_class[-1].bias.data)
        model_to_check = self.model.module if isinstance(self.model, DistributedDataParallel) else self.model
        try:
            # Access the specific layer - check if your model structure matches this path
            bias_data = model_to_check.detection_transformer.ctrl_point_class[-1].bias.data
            print("DEBUG: Bias of last ctrl_point_class layer AFTER load:", bias_data)
            print("DEBUG: Bias checksum (sum):", bias_data.sum().item()) # Print sum as a simple check
        except AttributeError as e:
            print(f"DEBUG: Could not access specified layer for bias check: {e}")
        if resume and self.checkpointer.has_checkpoint():
            self.start_iter = checkpoint.get("iteration", -1) + 1
            # Reset iterator state when resuming
            if hasattr(self, "_data_loader_iter"): # Check if iterator exists (it should in your train_loop version)
                 self._data_loader_iter = iter(self.data_loader) # Reset using self.data_loader
        else:
            # If not resuming or no checkpoint, ensure start_iter is 0
            self.start_iter = 0
             # Also reset iterator if starting fresh
            if hasattr(self, "_data_loader_iter"):
                 self._data_loader_iter = iter(self.data_loader)
